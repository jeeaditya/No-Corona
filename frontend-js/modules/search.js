    ...
    renderResultsHTML(posts){
        if(posts.length){
            ...
            ${ posts.map(post=>{
              ...
              <span class="text-muted small">by ${post.author.username} <i>(${post.author.occupation})</i> on ${postDate.getDate()}/${postDate.getMonth()}/${postDate.getFullYear()}</span>
            </a>`
            }).join('') }
            
          </div>`)
        }else {
            this.resultsArea.innerHTML = `<p class="alert alert-danger text-center shadow-sm" >Sorry We cannot find any results.</p>`;
        }
        this.hideLoaderIcon();
